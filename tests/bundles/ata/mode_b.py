...
class Pipeline:
    def __init__(self, in_shape, out_shape, config):
        ...
        self.module.mode_b = bl.module(bl.modeb, config, input, ot=bl.f32, telescope=bl.ata)
    ...
    def transfer_out(self, buffer):
        self.copy(self.output.buffer, self.module.mode_b.get_output())
...
if __name__ == "__main__":
    ...
    in_shape = (20, 192, 8192, 2)
    out_shape = (2, 192, 8192, 1)
    ...
    for ibeam in range(out_shape[0]):
        phased = (bl_input_buffer * py_output_phasors[ibeam][..., :]).sum(axis=0)

        for ichan in range(out_shape[1]):
            for isamp in range(out_shape[2]):
                x = phased[ichan, isamp, 0]
                y = phased[ichan, isamp, 1]
